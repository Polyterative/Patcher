...
-- a private image storage bucket, and an atomic media reorder RPC. No backfills.
...
create table if not exists public.marketplace_listings (
  ...
  constraint marketplace_listings_shipping_options_length
    check (cardinality(shipping_options) <= 12),
...
create table if not exists public.listing_media (
  ...
  constraint listing_media_supported_mime_type
    check (mime_type in ('image/jpeg', 'image/png', 'image/webp')),
  constraint listing_media_position_range check (position >= 0 and position < 8),
  constraint listing_media_url_proxy
    check (url ~ '^https://images\.patcher\.xyz/marketplace-listings/'),
...
create index if not exists marketplace_listings_public_status_idx
...
comment on function private.is_marketplace_listing_public_safe(uuid) is
  'Returns true only for active or reserved listings whose seller profile is public. Module visibility is not an authorization boundary.';
...
comment on function private.is_marketplace_listing_sellable_by_owner(uuid, bigint) is
