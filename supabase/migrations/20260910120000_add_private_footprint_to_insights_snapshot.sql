-- Private footprint extension for get_application_insights_snapshot.
--
-- Preflight notes (internaldocs/patterns/BACKEND_METHODS.md §"Schema-change preflight"):
-- §1 N/A — SELECT-only function replace. No UPDATE, no backfill, no trigger
-- interaction, no `updated`-timestamp risk. Lock scope is the function only.
-- §2 No RLS/policy/grant change: signature, volatility, search_path, and the
-- revoke-from-public / grant-to-anon+authenticated statements are identical.
-- Static SQL only (no format()/dynamic SQL). Counts-only output: no ids,
-- author ids, names, timestamps, or per-entity rows ever leave the function.
-- Rollback: redeploy the previous function body.
...
as $$
  ...
  -- Global private-footprint totals. Each is one cheap count(*) over tables the
  -- RPC already scans (EXPLAIN-verified: bare seq-scan aggregates, sub-ms each
  -- on 10-20k-row synthetic data; the connected-patch semi-join matches the
  -- existing public_connected_patches plan shape). Public-side counts reuse the
  -- CTEs above so each population is scanned once for its total and once for
  -- its already-existing public slice. Placed after module_rows because WITH
  -- items can only reference earlier items.
  footprint_totals as (
    select
      (select count(*)::int from public.racks) as total_racks,
      (select count(*)::int from public_visible_racks) as public_racks,
      (select count(*)::int from public.modules) as total_modules,
      (select count(*)::int from module_rows) as public_modules,
      (
        select count(*)::int
        from public.patches p
        where exists (
      ...
      ) as total_connected_patches,
      (select count(*)::int from public_connected_patches) as public_connected_patches
  ),
  ...
  select
    jsonb_build_object(
      ...
      -- Private footprint: global totals + private slices derived as
      -- greatest(total - public, 0). `total_racks` counts ALL rack rows
      -- (unfiltered: a private rack "still there" is the mental model).
      -- Patches count connected works only (≥1 saved cable connection);
      -- unconnected drafts are in neither slice. Modules have no profile gate.
      'totalRacks', (select total_racks from footprint_totals),
      'privateRacks', greatest(
        (select total_racks - public_racks from footprint_totals),
        0
      ),
      'totalModules', (select total_modules from footprint_totals),
      'privateModules', greatest(
        (select total_modules - public_modules from footprint_totals),
        0
      ),
      'totalPatches', (select total_connected_patches from footprint_totals),
      'privatePatches', greatest(
        (select total_connected_patches - public_connected_patches from footprint_totals),
        0
      )
    ) as statistics,
    (
