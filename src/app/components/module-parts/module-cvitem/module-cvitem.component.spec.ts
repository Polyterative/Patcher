...
describe('ModuleCVItemComponent - Instance-Aware Highlighting', () => {
  ...
  /** Minimal mock of PatchDetailDataService — only selectedForConnection$ is needed */
  ...
  /** Factory: create a component instance with the given kind, CV, and instanceId */
  function createComponent(kind: 'in' | 'out', cv: CV, instanceId: number | undefined): ModuleCVItemComponent {
    const comp = new ModuleCVItemComponent(
      {} as any, // AppStateService — unused in highlight logic
      mockPatchService as any
    );
    // Set inputs (readonly bypass via Object.defineProperty for test)
  ...
  it('should highlight an input CV when cv.id AND instanceId both match', () => {
    ...
    // Select CV 42 on instance 501
    mockPatchService.selectedForConnection$.next({
      a: null,
      b: makeCVEntity(42, 501, 'in')
    });
  ...
  it('should NOT highlight an input CV on a different instance (same cv.id, different instanceId)', () => {
    ...
    // Select CV 42 on instance 501 — comp belongs to instance 502
    mockPatchService.selectedForConnection$.next({
      a: null,
      b: makeCVEntity(42, 501, 'in')
    });
  ...
  it('should NOT highlight an output CV on a different instance (same cv.id, different instanceId)', () => {
    ...
    // Select CV 7 on instance 501 — comp belongs to instance 502
    mockPatchService.selectedForConnection$.next({
      a: makeCVEntity(7, 501, 'out'),
      b: null
    });
  ...
  it('should highlight when both sides have undefined instanceId (0-instance module)', () => {
    ...
    mockPatchService.selectedForConnection$.next({
      a: null,
      b: makeCVEntity(42, undefined, 'in')
    });
  ...
  it('should NOT highlight when cv.id differs even if instanceId matches', () => {
    ...
    mockPatchService.selectedForConnection$.next({
      a: null,
      b: makeCVEntity(99, 501, 'in') // different CV id
    });
