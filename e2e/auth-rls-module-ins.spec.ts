...
/**
 * RLS contract test for public.module_ins.
 *
...
test.describe('RLS · module_ins', () => {
  ...
  let guardInputId: number | undefined;
  let insertedInputId: number | undefined;

  const inputRow = (name: string): {name: string; moduleid: number; min: number; max: number; authorid: string} => ({
    name,
  ...
  test.beforeAll(async () => {
    ...
    const {data: guard, error: guardError} = await authedClient
      .from('module_ins')
      .insert(inputRow(`rls-test-guard-${ randomUUID() }`))
      .select('id')
      .single();
    expect(guardError, `guard input insert failed: ${ guardError?.message }`).toBeNull();
    guardInputId = guard!.id;
  });

  test.afterAll(async () => {
    if (authedClient) {
      if (guardInputId) {
        await authedClient.from('module_ins').delete().eq('id', guardInputId);
      }
      if (insertedInputId) {
        await authedClient.from('module_ins').delete().eq('id', insertedInputId);
      }
      await authedClient.from('module_ins').delete().like('name', 'rls-test-%');
      await authedClient.auth.signOut();
    }
  });

  test('anon can SELECT (module catalog inputs stay publicly readable)', async () => {
    const {data, error} = await newAnonClient().from('module_ins').select('id, name, moduleid').limit(5);
    expect(error, `anon SELECT failed: ${ error?.message }`).toBeNull();
    expect(data?.length ?? 0, 'anon SELECT returned no rows — module catalog inputs would break').toBeGreaterThan(0);
  });

  test('authenticated user can SELECT (module catalog inputs stay readable)', async () => {
    const {data, error} = await authedClient.from('module_ins').select('id, name, moduleid').limit(5);
    expect(error, `authenticated SELECT failed: ${ error?.message }`).toBeNull();
    expect(data?.length ?? 0, 'authenticated SELECT returned no rows — module catalog inputs would break').toBeGreaterThan(0);
  });
  ...
  test('anon INSERT is blocked', async () => {
    ...
    const {data} = await newAnonClient().from('module_ins').select('id').eq('name', name).limit(1);
    expect(data?.length ?? 0, 'blocked anon INSERT leaked a row into module_ins').toBe(0);
  });

  test('anon UPDATE is blocked', async () => {
    expect(guardInputId).toBeTruthy();
    const {data: updated, error} = await newAnonClient()
      .from('module_ins')
      .update({name: 'rls-test-anon-update'})
      .eq('id', guardInputId!)
      .select('id');
    expect(error || (updated?.length ?? 0) === 0, 'anon UPDATE unexpectedly changed module_ins').toBeTruthy();

    const {data} = await authedClient.from('module_ins').select('name').eq('id', guardInputId!).single();
  ...
  test('anon DELETE is blocked', async () => {
    ...
    const {data: deleted, error} = await newAnonClient()
      ...
      .eq('id', guardInputId!)
      .select('id');
    expect(error || (deleted?.length ?? 0) === 0, 'anon DELETE unexpectedly removed a module_ins row').toBeTruthy();

    const {data} = await authedClient.from('module_ins').select('id').eq('id', guardInputId!).limit(1);
