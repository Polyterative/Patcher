...
| Worker adapter dispatch does not match imported adapter mix | Partially mitigated 2026-07-03: the local-only `snapshot-store-listings` Edge Function now dispatches WooCommerce Store API and Shopware metadata listings, so Schneider rows are eligible if the worker is later deployed. Shopify, BigCommerce, and custom metadata rows still rely on the local importer unless the worker is extended further. |
