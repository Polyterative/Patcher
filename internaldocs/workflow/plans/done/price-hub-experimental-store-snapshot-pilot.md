...
- Matcher review/strong thresholds and shared noise terms live in `store-configs.ts` as per-store match configuration defaults, so future source-specific tuning can happen beside the approved store entry instead of in `matcher.ts`. Found Sound's preorder suppression remains tag-only: preorder in a product title or handle is still penalized as product-identity noise.
...
- Module detail Store prices treats rows older than 14 days by `last_checked_at` as stale; legacy rows without `last_checked_at` use the latest snapshot `observed_at` as the UI freshness fallback.
...
- [x] Create `snapshot-store-listings` Edge Function with WooCommerce Store API and Shopware metadata adapters.
...
- [x] Add stale-data policy and safe missing-product deactivation gates. Snapshot retention remains separate.
- [x] Add read methods through `SupabaseService` for current listing summaries.
- [x] Add docs for adding a new store adapter (per-store crawl/import walkthrough + noise-term guidance). See [`scripts/price-hub/README.md`](../../../../scripts/price-hub/README.md).
- [x] Move worker adapter dispatch into `snapshot-store-listings` for the safe shared metadata adapters: WooCommerce Store API, BigCommerce metadata, and Shopware metadata now route through `_shared/price-hub/*` normalizers. BigCommerce completed 2026-07-07 by adding `bigcommerce_metadata` scheduled support without deploy/Cron changes. *(added 2026-07-03)*
- [x] Extend `snapshot-store-listings` dispatch for Shopify product JSON in the local-only worker path after validating scheduled endpoint semantics per store; deploy/Cron/manual invocation remain gated, and local crawler pagination/rate-limit behavior is still not assumed safe for the Edge Function. *(completed 2026-07-07; split from worker dispatch item 2026-07-07)*
- [x] Extend `snapshot-store-listings` dispatch for `custom` metadata in the local-only worker path after proving every custom store row is compatible with the shared metadata fetch/normalization path; deploy/Cron/manual invocation remain gated. *(completed 2026-07-07; split from worker dispatch item 2026-07-07)*
- [x] Reconcile local importer `next_check_at = now()` with the worker's daily/backoff schedule so turning cron on doesn't produce a 13k-row backlog against a 20/day cap. Completed 2026-07-07: local importer upserts now keep `last_checked_at`/`last_success_at` at import time but compute a deterministic future `next_check_at` spread across seven days from store/module/product identity. *(added 2026-07-03)*
- [ ] Add approved snapshot retention/compaction: keep detailed daily snapshots for 30 days, then compact older data into indefinite long-term statistics/checkpoints in a Home Assistant-style model. Move stable per-listing diagnostics (`priceHubAlternateMatchedModules`, `signalSoundsInventoryLocations`, etc.) from `module_price_snapshots.raw_meta` to `module_store_listings` so snapshots stay skinny. *(added 2026-07-03; approved 2026-07-07; policy clarified 2026-07-08)*
- [x] Move `NOISE_TERMS` and the magic 0.86 / 0.72 score thresholds in `matcher.ts` to per-store configuration in `store-configs.ts`. Preorder is a legitimate availability state — penalise it only when it appears in title/handle, not when it appears as a tag. Completed 2026-07-07 with store match defaults and Found Sound tag-vs-title/handle coverage. *(added 2026-07-03)*
- [x] Split `scripts/price-hub/catalog-crawler.ts` into a stable facade plus smaller crawler modules for WooCommerce, Shopify, sitemap metadata orchestration, Signal Sounds fallback inventory, sitemap helpers, shared helpers, and output writes. Completed 2026-07-07 without changing public imports. *(added 2026-07-03)*
- [x] Split I/O from pure logic in `import-local-snapshots.ts`. Completed 2026-07-07 by moving row planning, dedupe, conflict filtering, module-id helpers, product URL extraction, and next-check staggering into `scripts/price-hub/import-local/row-planning.ts` while preserving the `import-local-snapshots.ts` facade used by refresh/tests. *(added 2026-07-03)*
- [x] Add abortable fetch/body-read timeout guards to local crawler adapters so a hung store request cannot block a refresh indefinitely. Completed 2026-07-07 for WooCommerce, Shopify curl/product JSON, BigCommerce/Shopware/custom metadata sitemap paths, and Signal Sounds inventory overrides.
...
- [x] **P0:** Enable RLS on `stores`, `module_store_listings`, and `module_price_snapshots`; revoke write privileges from `anon` and `authenticated`; keep service-role crawler writes; add permissive public `select` policies so public read paths keep working. Pair with `SET search_path = public` on `tg_price_hub_set_updated_at` to clear the Price Hub `function_search_path_mutable` advisor warning.
- [x] Extend the FX map (`AUD`, `CAD`, `JPY` at minimum, plus any future store currency) in both `module-price-listings-card.utils.ts` and `module-price-summary.utils.ts`; then consolidate the two duplicates into one shared module labelled as an estimate only. Completed 2026-07-07 with shared frontend estimated EUR helper covering AUD, CAD, CHF, EUR, GBP, JPY, NOK, and USD.
- [ ] Make Shopify/BigCommerce/Shopware `parseDecimalPriceMinor` currency-aware so zero-decimal currencies (JPY, KRW, HUF, ISK, VND, CLP) are stored as true minor units. This requires a coordinated approved backfill/migration for the 758 existing Clockface Modular JPY rows before the frontend `/100` cancellation can be removed.
- [x] Add a stale/freshness rule to the module detail Store prices card: badge or hide rows whose `last_checked_at` is older than a documented threshold (start with 14 days) and surface "Last seen" instead of a current price. Completed 2026-07-07: stale rows remain visible in All results but show `Last seen`/`Stale data`, are excluded from current price/best-price comparisons, and sort after current known prices. `cacheBust(['priceHub'])` remains app-runtime-only; local importer scripts cannot safely trigger an Angular in-memory/localStorage cache bust after remote writes.
- [x] Deactivate listings whose product URL disappears from a completed local full-catalog crawl. Completed 2026-07-07: local refresh passes all crawled product URLs to the importer and deactivates missing active listings only when no product cap/explicit bound was used and coverage is at least 25 products plus 5 accepted import rows. Dry-run reports would-deactivate counts with a read key; standalone import requires explicit `--full-catalog` evidence. The compact reason is stored in `module_store_listings.last_error` and the row is marked `verification_status = stale`.
- [x] After every completed full-catalog crawl, mark previously-active listings whose product URL is no longer present as `active = false` (with a "not-seen-since" reason), gated by minimum pages/products so a partial crawl cannot wipe a store. Completed 2026-07-07 via local importer/refresh full-catalog evidence gates.
...
- [x] Keep `Search on` chips for unverified/untracked stores. **Completed 2026-07-07:** retailer chips are suppressed only for verified/current Price Hub listings; candidate, stale, and untracked stores remain visible as search fallbacks.
- [x] Extend existing `Search on > Other stores` chips for the imported Price Hub store batch with straightforward site-search URLs. **Completed 2026-07-07:** added Detroit Modular, Nightlife Electronics, Clockface Modular, Moog Audio, Noisebug, Pusherman Productions, Thonk, After Later Audio, Patch Point, ALM / Busy Circuits, Instruo, WMD, Michigan Synth Works, RobotSpeak, Cicada Sound, Intellijel, Schlappi Engineering, Zlob Modular, Soundium, Nano Modules, and Dreadbox; skipped stores where the URL was not part of this safe batch rather than adding a new UI surface.
...
- [x] Add sparse price history in the module Price Hub summary/header first. **Completed 2026-07-07:** read-only snapshot-history query/model/helper/UI code appends a subtle 60-day trend label to the existing module title price badge when enough eligible normalized EUR snapshots exist; the query suppresses the badge rather than showing a capped/truncated history sample, and tooltip copy labels it as sparse estimated-EUR history. No migrations, RLS/policy changes, Edge Function deploy, Cron, release, push, or data writes were performed.
- [ ] Remove the report-wrong-product UI prototype and restore whole listing rows as click targets for the external store listing. Wrong-match handling should stay internal/operator-owned, not a user reporting flow.
- [x] Derive a stable store hero color from each store slug hash while preserving the original hand-tuned `STORE_HERO_COLORS` overrides. Completed 2026-07-07.
...
| Snapshot table grows forever | Keep detailed daily snapshots for 30 days, then compact older data into indefinite long-term statistics/checkpoints before expanding beyond pilot. |
| Production backend risk | Additive schema only; RLS/policy/GRANT changes require explicit approval and advisor checks. The P0 Price Hub RLS gate was approved and applied 2026-07-06. |
| Schema-only tables have RLS disabled | Resolved 2026-07-06: RLS is enabled on all three Price Hub tables, anon/authenticated have public read only, and service_role retains crawler writes. |
| Anon key currently has full DML on Price Hub tables (RLS off + default `public.*` grants) | Resolved 2026-07-06 by migration `secure_price_hub_rls`: anon/authenticated now have only `SELECT`, public read policies exist, and service_role write grants remain for local imports. |
| Worker adapter dispatch does not match imported adapter mix | Locally mitigated 2026-07-07: the local-only `snapshot-store-listings` Edge Function now dispatches WooCommerce Store API, Shopify product JSON, BigCommerce metadata, Shopware metadata, and custom metadata listings, so imported pilot rows are eligible if the worker is later deployed. This is not deployed; Supabase Cron remains declined/manual-only. |
...
| Non-EUR/USD/GBP/CHF listings silently dropped from sorting and market-price rollup | Resolved 2026-07-07 for frontend display/sorting: the duplicate FX maps were consolidated into `module-price-estimated-fx.utils.ts` and seeded AUD/CAD/JPY/NOK stores now participate in estimated EUR ordering and rollups. Currency-aware storage/backfill remains separate. |
| Stale/freshness policy documented but not enforced on read | Resolved 2026-07-07 for the module detail card: rows older than the 14-day freshness threshold render as `Last seen`/`Stale data`, do not expose a current price, and do not participate in best-price ranking. Backend stale status/retention remains separate. |
| Product disappearance from store catalog is invisible | Resolved 2026-07-07 for local full-catalog refresh/imports: compare the complete crawled product URL set to active listings and mark missing rows `active = false`, `verification_status = stale`, with compact `last_error = not_seen_since_full_catalog:YYYY-MM-DD`. Partial/bounded/low-coverage crawls skip this path, and standalone import requires explicit `--full-catalog`. |
| Anon/publishable keys accidentally used for live imports | Resolved 2026-07-06: live import/refresh now default to service-role key aliases only and reject anon/authenticated JWT keys before attempting writes. |
| Local crawler credentials live outside this checkout | Resolved 2026-07-06 for local operation: `PRICE_HUB_ENV_FILE` can point to an external env file, so secrets do not need to be copied into the Patcher repository. |
| `--max-products` did not cap Shopify/WooCommerce crawls | Resolved 2026-07-07: Shopify and WooCommerce adapters honor `maxProducts` and report `hitMaxProducts`; broad live `--store=all` partial refreshes stay `BLOCKING`, while dry-runs or explicit single-store bounded refreshes can import with a warning. |
| Hung store fetches/body reads block local refreshes | Resolved 2026-07-07: local crawler adapters pass AbortSignals to fetches, timeout body reads with response aborts, and give the Shopify curl fallback an exec timeout/signal so blocked stores fail clearly instead of hanging. |
...
- Manually invoke the Edge Function before any future scheduling work.
- Keep Supabase Cron disabled unless the user later changes the manual/on-demand decision.
...
- [x] User approval for Price Hub RLS/policy/GRANT changes.
- [x] User approval to deploy/update the Edge Function after local tests and configuration checks pass.
...
- [x] User approval for zero-decimal storage/backfill changes for Clockface Modular JPY and other zero-decimal currencies.
- [x] User approval for snapshot retention/compaction and moving stable diagnostics out of `raw_meta`.
- [x] User decision on report-wrong-product persistence/UI: persistence is declined, the prototype UI should be removed, and whole listing rows should again open the external store listing.
- [x] User approval for sparse price history sparklines in the module Price Hub summary/header.

### Approval queue

- **Approval recorded 2026-07-06T19:34+02:00 (P0):** User approved public reads for anon/authenticated users, RLS write protection, service-role/administrator crawler writes, and setting up the local credential path needed for clean database writes.
...
- **Approval recorded 2026-07-07T14:07+02:00 — zero-decimal storage/backfill:** User approved a safe additive zero-decimal storage migration/backfill with validation queries/checks before and after.
- **Approval recorded 2026-07-07T14:07+02:00 — retention + diagnostics:** User approved additive retention/diagnostics schema/backend work for snapshot retention state, scrape diagnostics, and failure metadata.
- **Decision recorded 2026-07-08T14:13+02:00 — wrong-product flagging:** User declined user-facing wrong-product reports. Remove the report UI prototype, restore whole-row external click-through, and keep wrong-match handling as internal/operator data hygiene. Do not add `module_store_listing_flags`, RLS, backend persistence, or user reporting UX by default.
- **Approval recorded 2026-07-07T14:07+02:00 — sparse sparkline:** User approved sparse price-history sparklines in the module Price Hub summary/header.
- **Approval recorded 2026-07-07T14:07+02:00 — Edge/Cron:** User approved Edge Function deploy/update after validation. Supabase Cron is declined for now; keep snapshots manual/on-demand.
