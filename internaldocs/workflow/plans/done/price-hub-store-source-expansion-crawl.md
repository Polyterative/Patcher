...
| Whimsical Raps | whimsicalraps.com | United States | USD | Shopify `products.json` | imported | robots.txt identifies Shopify and products.json returned 200 with product JSON. A 2026-07-07 targeted refresh recovered from local curl TLS trust-chain failures by retrying with `--ca-native` and imported 5 snapshots/listings. |
...
| Martin Pas | martinpas.com | Netherlands | EUR | custom category metadata | imported | Follow-up QA used the public `/categories/modular-systems` page rather than the broad sitemap. Product pages expose brand, currency, availability, and embedded minor-unit prices; 87 products crawled and 30 snapshots imported on 2026-07-03. |
...
| Turnlab | turnlab.be | Belgium | EUR | Lightspeed category metadata | imported | Follow-up QA used the public Eurorack category pages. Product pages expose schema.org price/brand metadata and Add-to-cart availability signals; 61 products crawled and 28 snapshots imported on 2026-07-03. |
...
| TechnoSynth | technosynth.com | Canada | CAD | WooCommerce Store API | currently blocked | robots.txt and WooCommerce Store API previously returned 200 with product JSON including Buchla module data. A 2026-07-07 refresh/probe now returns Cloudflare 403 challenge HTML even with crawler headers; do not bypass bot protection. |
