...
| [`coordinator-loop.md`](./coordinator-loop.md) | `executor` | Triggered by "begin loop" / "run the loop"; selects one task, delegates implementation and review, validates, commits verified chunks, and archives docs. | Completed TODO → plan → implementation → review → verified commits → COMPLETED loop. Heavier than most personas by design (full loop mission prompt) — load it once instead of pasting. |
